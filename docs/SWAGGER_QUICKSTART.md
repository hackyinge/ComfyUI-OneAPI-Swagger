...
#### 测试 Gemini 兼容接口 (Google AI SDK)

1. 展开 `POST /oneapi/v1/models/{model}:generateContent` 接口
2. 点击 **"Try it out"**
3. `model` 填写你保存的工作流文件名（例如 `wenshengtu_api`）
4. 在 **Request body** 中输入 Gemini 标准请求格式：
  ...
  "contents": [
    {
       "role": "user",
       "parts": [
         {"text": "a cute puppy running on grass"}
       ]
    }
  ]
}
...
5. 执行，响应将包含符合 Gemini 规范的 Base64 图片数据。
...
5. **Gemini 兼容** - 该接口专为 Google AI SDK 等 LLM 框架设计，返回 Base64 格式图片，方便直接嵌入对话系统。
